 ...
 */
function makeBuilder(attachmentLabels: Array<string> = []): { builder: IMessageBuilder; getRemainingLabels: () => Array<string> } {
...
test('when quarantine is unavailable and multiple attachments matched, removes exactly those and nothing else', async () => {
    ...
    await handleMatchingMessage(
        [makeMatchResult('second.jpg', 'second-marker'), makeMatchResult('fourth.jpg', 'fourth-marker')],
        [1, 3],
        makeMessage(),
        makeRead(),
        {} as IPersistence,
