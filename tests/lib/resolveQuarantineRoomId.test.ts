...
import type { ILogger, IRead } from '@rocket.chat/apps-engine/definition/accessors';
...
function makeRead(room: IRoom | undefined): { read: IRead; getByNameCalls: Array<string> } {
    const getByNameCalls: Array<string> = [];
    const read = {
        getRoomReader: () => ({
            getByName: async (name: string) => { getByNameCalls.push(name); return room; },
        }),
    } as unknown as IRead;
...
test('logs at error severity and returns undefined when the configured channel does not exist', async () => {
    ...
    const roomId = await resolveQuarantineRoomId('typo-channel-name', read, logger);

    assert.equal(roomId, undefined);
    assert.equal(errorCalls.length, 1);
    assert.ok(errorCalls[0].some((arg) => typeof arg === 'string' && arg.includes('typo-channel-name')));
});
