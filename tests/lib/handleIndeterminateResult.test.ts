...
function makeBuilder(attachmentLabels: Array<string> = ["attachment"]): {
    ...
    return {
        ...
        getRoomCalls,
        getText: () => text,
        getRemainingLabels: () => attachments,
    };
}
...
test("logs at error severity and includes the reason", async () => {
    ...
    await handleIndeterminateResult(
        "No response was received from the PhotoDNA API.",
        [0],
        makeMessage(),
        makeRead(undefined),
        builder,
    ...
    assert.ok(
        errorCalls[0].some(
            (arg) =>
                typeof arg === "string" &&
                arg.includes("No response was received from the PhotoDNA API."),
        ),
    );
});
...
test("prepends a notice naming the reason to the message text", async () => {
    ...
    assert.match(
        getText(),
        /^PhotoDNA verification failed \(A network error occurred\.\)/,
    );
...
test("logs at error severity when the configured quarantine room can no longer be found", async () => {
    ...
    assert.equal(
        errorCalls.length,
        2,
        "expected one error for the verification failure and one for the missing quarantine room",
    );
    assert.ok(
        errorCalls.some((call) =>
            call.some(
                (arg) =>
                    typeof arg === "string" && /could not be found/i.test(arg),
            ),
        ),
    );
});
