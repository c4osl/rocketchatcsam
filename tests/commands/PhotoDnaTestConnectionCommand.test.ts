...
test('rejects a non-admin user without calling the API', async () => {
    ...
    const http = { post: async () => { httpCalled = true; return { data: {} }; } } as unknown as IHttp;

    await command.executor(makeContext(['user']), makeRead('fake-key'), modify, http, {} as IPersistence);
...
test('reports when the API key setting is not configured', async () => {
    ...
    await command.executor(makeContext(['admin']), makeRead(undefined), modify, http, {} as IPersistence);
...
test('reports success for an admin when the API responds with a match', async () => {
    ...
    await command.executor(makeContext(['admin']), makeRead('fake-key'), modify, http, {} as IPersistence);
...
test('reports failure for an admin when the API rejects the key', async () => {
    ...
    await command.executor(makeContext(['admin']), makeRead('invalid-key'), modify, http, {} as IPersistence);
