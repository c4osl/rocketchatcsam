...
export class PhotoDnaTestConnectionCommand implements ISlashCommand {
    ...
    public async executor(
        ...
        if (!apiKey) {
            await reply(
                'The "API Subscription Key" setting is not configured yet.',
            );
            return;
        }

        const logger = makeSilentLogger();
        const outcome = await this.photoDnaService.checkConnection(
            http,
            read,
            logger,
            getPhotoDnaTestImageBuffer(),
        );
...
function describeResult(outcome: MatchOutcome): string {
    ...
    const source =
        outcome.result.MatchDetails?.MatchFlags?.[0]?.Source ?? "n/a";
